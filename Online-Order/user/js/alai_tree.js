...
var aid=0;
var addNodeId=0;
Function.prototype.closure = function(nodeId, evtId) {
    __funcs.push(this);
	fid[nodeId*100+evtId]=aid++;
    return function () {
		return __funcs[fid[nodeId*100+evtId]].apply(null, arguments);
    };
};
...
{try{
	...
	var addNode=function(toNode,relation,text,key,ico,exeCategory,exeArg)
	{
		...
		with(label)
		{
			onclick=function(){run(node.select);if(run(node.onclick))if(run(tree.onclick,node))run(node.execute);}.closure(addNodeId,1);
			ondblclick=function(){if(run(node.ondblclick))run(tree.ondblclick,node)}.closure(addNodeId,2);
			onmouseover=function(){if(event.fromElement!=node.icon)if(run(node.onmouseover))run(tree.onmouseover,node)}.closure(addNodeId,3);
			onmouseout=function(){if(event.toElement!=node.icon)if(run(node.onmouseout))run(tree.onmouseout,node);}.closure(addNodeId,4);
			onmousemove=function(){if(run(node.onmousemove))run(tree.onmousemove,node)}.closure(addNodeId,5);
			onmousedown=function(){if(run(node.onmousedown))run(tree.onmousedown,node)}.closure(addNodeId,6);
			onmouseup=function(){if(run(node.onmouseup))run(tree.onmouseup,node)}.closure(addNodeId,7);
			onkeypress=function(){if(run(node.onkeypress))run(tree.onkeypress,node)}.closure(addNodeId,8);
			onkeydown=function(){if(run(node.onkeydown)){if(run(tree.onkeydown,node))doKeyDown(node);}}.closure(addNodeId,9);
			onkeyup=function(){if(run(node.onkeyup))run(tree.onkeyup,node)}.closure(addNodeId,10);
			onfocus=function(){if(run(node.onfocus)){if(run(tree.onfocus,node))doFocus(node);}}.closure(addNodeId,11);
			onblur=function(){if(run(node.onblur)){if(run(tree.onblur,node))doBlur(node);}}.closure(addNodeId,12);
		}
		//with(icon)
		//{
			icon.onclick=function(){run(node.select);if(run(node.onclick))if(run(tree.onclick,node))run(node.execute);}.closure(addNodeId,13);
			icon.ondblclick=function(){if(run(node.ondblclick))run(tree.ondblclick,node)}.closure(addNodeId,14);
			icon.onmouseover=function(){if(event.fromElement!=node.icon)if(run(node.onmouseover))run(tree.onmouseover,node)}.closure(addNodeId,15);
			icon.onmouseout=function(){if(event.toElement!=node.icon)if(run(node.onmouseout))run(tree.onmouseout,node);}.closure(addNodeId,16);
			icon.onmousemove=function(){if(run(node.onmousemove))run(tree.onmousemove,node)}.closure(addNodeId,17);
			icon.onmousedown=function(){if(run(node.onmousedown))run(tree.onmousedown,node)}.closure(addNodeId,18);
			icon.onmouseup=function(){if(run(node.onmouseup))run(tree.onmouseup,node)}.closure(addNodeId,19);
			icon.onkeypress=function(){if(run(node.onkeypress))run(tree.onkeypress,node)}.closure(addNodeId,20);
			icon.onkeydown=function(){if(run(node.onkeydown)){if(run(tree.onkeydown,node))doKeyDown(node);}}.closure(addNodeId,21);
			icon.onkeyup=function(){if(run(node.onkeyup))run(tree.onkeyup,node)}.closure(addNodeId,22);
			icon.onfocus=function(){if(run(node.onfocus)){if(run(tree.onfocus,node))doFocus(node);}}.closure(addNodeId,23);
			icon.onblur=function(){if(run(node.onblur)){if(run(tree.onblur,node))doBlur(node);}}.closure(addNodeId,24);
		//}
		nbody.onblur=function(){if(node.onfocus()){if(tree.onfocus(node))doFocus(node);}}.closure(addNodeId,25);
		nbody.onfocus=function(){if(node.onblur()){if(tree.onblur(node))doBlur(node);}}.closure(addNodeId,26);
		...
		var ma=node.parent;if(ma!=root){if(ma.exIcon==null && showExpand){ma.exIcon=new Image();var o=ma.icon.src==""?ma.label:ma.icon;o.insertAdjacentElement("beforeBegin",ma.exIcon);}setExIcon(ma);if(ma.exIcon){ma.exIcon.onmousedown=function(){ma.expand();}.closure(addNodeId,27);}}
