...
function openMaxWin(winname,url){
  window.open(url,winname,'top=0,left=0,toolbar=0,status=0,location=0,menubar=0,scrollbars=1,resizable=1,width='+(screen.availWidth-10)+',height='+(screen.availHeight-50));
  return;
}
...
function openCalendar(viewvar,retvar,ismonth){
  var options = "width=380,height=320";
  options += "resizable=yes,scrollbars=no,status=yes,";
  options += "location=no,menubar=no,toolbar=no,directories=no";
  var url = '../common/calender.htm?viewvar='+viewvar+'&retvar='+retvar;
  if(ismonth!=null){
    url += ('&ismonth='+ismonth);
  }
