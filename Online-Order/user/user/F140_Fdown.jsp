...
function change(n){
   ...
   //执行更新动作
   function update(){
    if(group1.document.forms[0].timetype.value == "" && group1.document.forms[0].iptype.value == ""){
    	alert("请先加入限制条件！");
        group1.document.forms[0].timetype.focus();
        return;    	
    }
    if(group1.document.forms[0].timetype.value!="" && group1.document.forms[0].limitedtime.length=="0"){
    	alert("请选择加入一个时间段！");
        group1.document.forms[0].timetype.focus();
        return;    	
    }
    if(group1.document.forms[0].iptype.value!="" && group1.document.forms[0].limitedip.length=="0"){
    	alert("请选择加入一个ip地址！");
        group1.document.forms[0].iptype.focus();
        return;    	
    }
    
    for(var m=0;m<group1.document.forms[0].limitedtime.length;m++){
        group1.document.forms[0].limitedtime.options[m].selected=true;
    }
    for(var n=0;n<group1.document.forms[0].limitedip.length;n++){
        group1.document.forms[0].limitedip.options[n].selected=true;
    }

     group1.document.forms[0].op.value = "update";
     group1.document.forms[0].forward.value = "Upd_down";
...
<!--Title of card End -->
	...
	<input type="button" name="save" value="保  存" onclick="update()" class="button"/>
        </td>
	</tr></table>
