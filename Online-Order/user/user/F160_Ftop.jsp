...
//title卡片标题的名称，Ltitle卡片标题栏的宽度，Ntitle卡片标题的个数,UrlLay卡片对应的层的页面，HeiLay层的高度
	String[] title = {"在线用户"};
	String[] Ltitle = {"80"};
	String[] UrlLay = {request.getContextPath() + "/user/F160_UserOnlAction.do?forward=Qur_top&op=select"};
	String[] HeiLay = {"246"};
	int Ntitle = 0;
	if(title!=null){
		Ntitle = title.length;
...
<script LANGUAGE="JavaScript" type="">
    ...
    function adduser(){
        group1.change_color();
        parent.Frame2.document.all.group1.src = <%=com.conant.ums.util.ComGlobal.CONTEXTROOT%> + "user/F160_UserOnlAction.do?forward=Insertdown&op=selectoptions";
    }

    //弹出查询窗口
    function open_select(){
        var openurl = 'F160_select.jsp';
...
<!--Title of card End -->
      <td height="23"  ID="t3"  class="tdright">
        ...
        <select name="curPageNo" onchange="chg_pageno()" style="width:75" Class="SelectButton"></select>
      </td>
      </tr>
      </table>
<!--div of card Start-->
