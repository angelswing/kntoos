...
//title卡片标题的名称，Ltitle卡片标题栏的宽度，Ntitle卡片标题的个数,UrlLay卡片对应的层的页面，HeiLay层的高度
	String[] title = {"角色管理"};
	String[] Ltitle = {"80"};
	String[] UrlLay = {request.getContextPath() + "/user/F110_RoleMgtAction.do?forward=SelectRecord&op=select"};
	String[] HeiLay = {"246"};
	int Ntitle = 0;
	if(title!=null){
		Ntitle = title.length;
...
function doSelect(){
        var url = "<%=request.getContextPath()%>/user/F110_RoleMgtAction.do?forward=QurCondition&op=selectoptions";
        window.open(url,"_query","width=400,height=200,top=259, left=312, toolbar=no,scrollbars=yes,location=no, status=no");    
}

function doInsert(){
    group1.change_color();
    parent.Frame2.document.all.group1.src = "<%=request.getContextPath()%>/user/F110_RoleMgtAction.do?forward=AddDept&op=selectoptions";
}

function doDelete(){
    if ( group1.document.forms[0].selectedrole_id.value != "" ){
        if ( confirm("确定删除该记录吗？")) {
    		group1.doDelete();
        }
    }
...
function selectInfo(){
    group1.document.forms[0].selectedrole_id.value = "";
...
<!--Title of card End -->
	...
	<input type="button" name="equry" value="查  询" class="button" onclick="doSelect()"/>
